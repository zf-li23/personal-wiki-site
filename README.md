...
这是一个基于 React, Vite 和 Tailwind CSS 构建的个人知识库网站。现在只保留 wiki 这一项功能，首页也只是知识库的导航入口。设计目标是易于部署到 GitHub Pages，并提供类似 GitBook 的阅读体验。

## 功能特点

-   **维基系统**: 支持无限层级的文档结构，自动生成侧边栏导航，首页会把一级目录放大展示。
...
│   ├── components/      # UI 组件
│   ├── data/
│   │   └── wikiData.ts  # 自动读取 docs/ 下的 md 文件并生成路由数据
│   ├── pages/           # 页面组件
│   └── ...
├── ...
```
...
1.  **文件夹结构**: 在 `docs/` 下创建文件夹，文件夹名称必须包含编号（如 `1.离散数学`）。
