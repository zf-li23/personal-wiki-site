...
**定义 4.1.3** 在集合 $Z$ 上定义小于等于关系 $\leq$ 为，对任意的 $x,y \in Z$，$x \leq y$ 当且仅当
$$
...
**定义 4.1.4** 等价关系 $\approx$ 对整数集合 $Z$，令
$$
...
**定理 4.1.1** 在 $Q_1$ 上的关系 $\approx$ 是等价关系。

**定义 4.1.5** 有理数集合 令 $Q = Q_1/\approx$，即 $Q$ 是集合 $Q_1$ 对等价关系 $\approx$ 的商集，则称 $Q$ 的元素为有理数，一般用 $a/b$ 表示 $Q$ 中的元素 $[a,b]_≈$。并习惯上取 $a, b$ 是互素的整数，且 $b > 0$。

**定义 4.1.6** 在 $Q$ 上定义小于等于关系 $\leq$ 为，对任意的 $a/b, c/d \in Q$，
$$
a/b \leq c/d \text{ 当且仅当 } a \cdot d \leq b \cdot c。
$$

**定义 4.1.7** 基本函数 如果 $f:N \rightarrow Q$ 满足条件：
...
**定理 4.1.2** 当 $f: N \to Q$ 取常数值时，$f$ 是基本函数。即对任意的 $r \in Q$，
$$
r, r, r, \cdots
$$
是一个基本序列。

**定理 4.1.3** 存在不是常值函数的基本函数。

**定义 4.1.8** 对基本函数的集合 $B$，定义 $B$ 上的关系 $\sim$ 为，对任意的 $f, g \in B$，$f \sim g$ 当且仅当
$$
...
**定理 4.1.4** $B$ 上的关系 $\sim$ 是等价关系。

**定理 4.1.5** 设 $f: N \to Q$ 和 $g: N \to Q$ 都是常值函数，且 $f \sim g$，则 $f = g$。

**定义 4.1.9** 实数集 令 $R = B / \sim$，即 $R$ 是集合 $B$ 对等价关系 $\sim$ 的商集，则称 $R$ 的元素为实数，称 $R$ 为实数集合。

**定义 4.1.10** 在 $B$ 上定义小于关系 $<_B$ 为，对任意的 $f, g \in B$，$f <_B g$ 当且仅当
$$
...
**定义 4.1.11** 在 $R$ 上定义小于等于关系 $\leq_R$ 和小于关系 $<_R$ 为，对任意的 $f, g \in B$，
$$
...
**定义 4.2.1** 集合的等势 对集合 $A$ 和 $B$，如果存在从 $A$ 到 $B$ 的双射函数，就称 $A$ 和 $B$ 等势，记作 $A \approx B$。如果不存在从 $A$ 到 $B$ 的双射函数，就称 $A$ 和 $B$ 不等势，记作 $\neg A \approx B$。
...
**定义 4.3.1** （有限集合与无限集合） 集合 $A$ 是有限集合，当且仅当存在 $n \in N$，使 $n \approx A$。集合 $A$ 是无限集合当且仅当 $A$ 不是有限集合，即不存在 $n \in N$ 使 $n \approx A$。

**定理 4.3.1** 不存在与自己的真子集等势的自然数。

**推论 4.3.1** 不存在与自己的真子集等势的有限集合。

**推论 4.3.2** 任何与自己的真子集等势的集合是无限集合。$N$ 和 $R$ 都是无限集合。

**推论 4.3.3** 任何有限集合只与唯一的自然数等势。

## 4.4. 集合的基数

**定义 4.4.1** 对任意的集合 $A$ 和 $B$，它们的基数分别用 $\text{card}(A)$ 和 $\text{card}(B)$ 表示，并且 $\text{card}(A) = \text{card}(B) \Leftrightarrow A \approx B$。（有时把 $\text{card}(A)$ 记作 $|A|$ 或 $\#(A)$。） 对有限集合 $A$ 和 $n \in N$，若 $A \approx n$，则
$$
\text{card}(A) = n。
$$
...
**定义 4.6.1** 对集合 $K$ 和 $L$，$\text{card}(K) = k, \text{card}(L) = l$，如果存在从 $K$ 到 $L$ 的单射函数，则称集合 $L$ 优于 $K$，记作 $K \preceq L$，且称基数 $k$ 不大于基数 $l$，记作 $k \leq l$。

**定义 4.6.2** 对基数 $k$ 和 $l$，如果 $k \leq l$ 且 $k \neq l$，则称 $k$ 小于 $l$，记作 $k < l$。

**定理 4.6.1** 对任意的基数 $k,l$ 和 $m$，
...
**定理 4.6.2** 对任意的基数 $k,l$ 和 $m$，如果 $k \leq l$，
...
**定理 4.6.3** 对基数 $k$ 和 $l$，如果 $k \leq l$，$k \neq 0$，$l$ 是无限基数，则
$$
...
**定义 4.7.1** （可数集合） 对集合 $K$，如果 $\text{card}(K) \leq \aleph_0$，则称 $K$ 是可数集合。
